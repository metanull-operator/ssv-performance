...
def create_operator_performance_message(operator_data):
    ...
    if FIELD_PERF_DATA_24H in operator_data and operator_data[FIELD_PERF_DATA_24H]:
        ...
        data_points = {date: performance for date, performance in operator_data[FIELD_PERF_DATA_24H].items() if date in last_x_days}
        data_points = dict(sorted(data_points.items(), key=lambda item: item[0], reverse=True))

        if data_points and data_points.keys():
            for data_date in data_points.keys():
                data_value = data_points[data_date]
                if data_value is not None:
                    message += f"- {data_date}: {data_value * 100:.2f}%\n"
                else:
                    message += f"- {data_date}: N/A\n"
        else:
            message = "24h Performance: N/A\n"
    else:
        message += "24h Performance: N/A\n"
...
def compile_operator_performance_messages(perf_data, operator_ids):
    ...
    for operator_id in reporting_ids:
        msg = create_operator_performance_message(perf_data[operator_id])
        if (msg):
            messages.append(msg)
